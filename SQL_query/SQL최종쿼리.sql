	...
	WHERE A.DT_COMP BETWEEN '20240401' AND '20240930'										
	  AND A.CK_END <> '9'											
	  AND A.CD_PROCESS = '05'											
		...
		CASE										
			...
			WHEN 제대타입 IN ('삼방꼬리', '삼방꼬리-HD씨링') THEN									
				CASE								
					...
					ELSE 0							
				END								
			WHEN 제대타입 IN ('삼방-지퍼', '삼방스탠드', '2방스탠드', '삼방-지퍼-2폭', '2단삼방-지퍼', '삼방-포켓지퍼', '2단삼방-풀발이') 
			THEN CAST(품목너비 / 10 AS INTEGER)									
			WHEN 제대타입 IN('삼방스탠드-지퍼', '2단삼방스탠드-지퍼') THEN CAST(품목너비 / 10 * 1.4 AS DECIMAL(10, 0))									
			ELSE 0									
			END									
			AS '기본단가'									
		...
		*,		
		...
		CASE										
			WHEN 품목타입 LIKE '%HD씨링%' OR 단가수정key LIKE '%HD씨링%' THEN									
				CASE								
					WHEN 품목타입 LIKE '%2폭%' OR 단가수정key LIKE '%2폭%' THEN (기본단가 + 0.5-1)							
					ELSE (기본단가 + 0.5)							
				END								
		...
		END AS 최종단가	
		FROM analysis2	
		-- WHERE 주문번호 = '20240529-1000159459'
